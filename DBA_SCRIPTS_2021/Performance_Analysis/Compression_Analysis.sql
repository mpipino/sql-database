...
SELECT DB_NAME() Base, 
    ...
    SUM(a.used_pages) * 8 AS UsedSpaceKB, SUM(a.used_pages) * 8 / 1024 / 1024 GBTabla
INTO #tblGrandes
FROM sys.tables t
...
SELECT 
--'ALTER INDEX  ' + i.name + ' ON '+ t.TableName + ' REBUILD PARTITION = ' + convert(varchar, sp.partition_number) + ' WITH (DATA_COMPRESSION=PAGE);
'ALTER INDEX  ' + i.name + ' ON '+ t.TableName + ' REBUILD WITH (DATA_COMPRESSION=PAGE, ONLINE=ON);
WAITFOR DELAY ''00:00:01'''
from #tblGrandes t
join sys.indexes i on t.OBJECT_ID = i.OBJECT_ID--89
join sys.partitions SP on sp.object_id = i.object_id and sp.index_id = i.index_id
--where t.TableName = 'tblhistoriaDetalleHistoria'
where sp.data_compression_desc <> 'PAGE'
--AND sp.partition_number <> 1
order by GBTabla desc
...
--WHERE o.type = 'U' AND i.[index_id] >0 and o.name='tbl_distributor_commissions_v2' 
